...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['name'])) {
        ...
        // 插入采购信息
        $sql = "INSERT INTO supplier (name, sex, telephone, email, firm) VALUES ('$name', '$sex', '$telephone', '$email', '$firm')";
...
// 根据查询方法执行不同的查询操作
    if ($searchMethod === 'name') {
        // 根据用户名进行查找
        $sql = "SELECT * FROM supplier WHERE name LIKE '%$searchKeyword%'";
    } else if ($searchMethod === 'firm') {
        // 根据权限等级进行查找
        $sql = "SELECT * FROM supplier WHERE firm = '$searchKeyword'";
    } else {
        // 如果查询方法未知，返回错误消息
...
// 获取数据
    ...
    $sql = "SELECT id, name, sex, telephone, email, firm FROM supplier";
    $result = $conn->query($sql);
